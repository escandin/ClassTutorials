---
title: 'Applied Machine Learning for Spatial Analysis. Lab Week 5: Unsupervised classification'
...
## Goal
To learn how to perform a spatial supervised classification and how to interpret the results.
...
#### System setup
Define working directory and load required libraries. Install libraries as needed. Define folder to save temporary files
```{r include = TRUE, message=F, warning=F, eval=FALSE}
wd=("/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/MALESA/2022Fall/Week5")
setwd(wd)
dir()
...
terraOptions(tempdir="tempdir", progress=5)
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
for(xval in 1:ncol(df)){
    df[,xval]=zscore(df[,xval])
  }
```
...
#### Run the k-means analysis using the k-means function
```{r include = TRUE, message=F, warning=F, eval=FALSE}
...
#### Bonus: Include coordinates as variables in the cluster
```{r include = TRUE, message=F, warning=F, eval=FALSE}
df$x=coord[,1]
df$y=coord[,2]
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
clusters$cluster
clustersGeo$cluster # for bonus
```

## Lab instructions (due on Sep 26)

1. Select three variables that you believe, will likely have the strongest influece on the variable that you want to predict in your final project. Briefly describe the rationalle for the selection of the variables.
