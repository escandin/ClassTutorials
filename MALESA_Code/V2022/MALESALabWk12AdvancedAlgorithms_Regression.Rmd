---
title: 'Applied Machine Learning for Spatial Analysis. Lab Week 10: Advanced algorithms (supervised regression)'
...
library(sf)
#library(pdp)
sourcedir="/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/MALESA/2022Fall/Week10"
```

Load required files
```{r include = TRUE, message=F, warning=F, eval=FALSE}
load(paste0(sourcedir,"/refdata.RData"))
names(refdata)
refdata=refdata[,-2]
```
...
trainx=data.frame(apply(train, 2, FUN=zscore))
testx=data.frame(apply(test, 2, FUN=zscore))
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
names(getModelInfo())
gnni=getModelInfo("neuralnet")
gnni$neuralnet$type
gnni$neuralnet$library
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
# Random forest
rf_model <- caret::train(x = train[,(2:ncol(train))], y = train$agb,
                         method = "rf", metric="RMSE", trainControl = tc, tuneGrid = rf.grid)
rf_model
plot(rf_model)

# XG boost
xg_model = train(agb~., data = train, method = "xgbTree", metric="RMSE", 
                 trainControl = tc, 
                tuneGrid = gbm.grid)
xg_model
plot(xg_model)
...
# neuralnet
# If the algorithm does not converge, you can increase threshold or stepmax
# https://stats.stackexchange.com/questions/183534/neural-network-model-does-not-converge 
neunet_model <- caret::train(x = trainx[,(2:ncol(trainx))], y = trainx$agb, 
  #formula.nps,
  #data = training.set,
  method = "neuralnet",
  metric = "RMSE",
  #threshold=0.02,
  linear.output = TRUE, 
  tuneGrid = neunet.grid, # cannot pass parameter hidden directly!!
  trControl = tc)
neunet_model

# nnet
nnet_model <- caret::train(x = trainx[,(2:ncol(trainx))], y = trainx$agb,
                           method = "nnet", metric="RMSE",#"Accuracy", 
...
# rf
sum(sqrt((test_y - pred_rf)^2))/length(test_y) #mae - Mean Absolute Error
caret::RMSE(test_y, pred_rf) #rmse - Root Mean Squared Error

# xgboost
sum(sqrt((test_y - pred_xg)^2))/length(test_y) #mae - Mean Absolute Error
caret::RMSE(test_y, pred_xg) #rmse - Root Mean Squared Error

# neunet
sum(sqrt((test_y - pred_neunet)^2))/length(test_y) #mae - Mean Absolute Error
caret::RMSE(test_y, pred_neunet) #rmse - Root Mean Squared Error

# nnet
sum(sqrt((test_y - pred_nnet)^2))/length(test_y) #mae - Mean Absolute Error
caret::RMSE(test_y, pred_nnet) #rmse - Root Mean Squared Error
```
...
names(datastack)==names(train)[2:ncol(train)]
names(datastack)[7]=names(train[8])
names(datastack)==names(train)[2:ncol(train)]

# The size is to big for this so let's subset it
plot(datastack[[15]])
e=draw("extent")
datastackrz=crop(datastack, e)
plot(datastackrz[[1]])

# convert datastack into dataframe
datastackrzdf=as.matrix(datastackrz, mode="numeric")
datastackrzdf=subset(datastackrzdf, complete.cases(datastackrzdf))
colnames(datastackrzdf)[7]='vpdm.1'

# Apply the models to the spatial data
# Let's create a reference raster template
refrast=sum(datastackrz)
refrast[!is.na(refrast)] <- 1

# Random forest
rf_prediction = predict(rf_model, datastackrzdf)
rf_rast=refrast
rf_rast[!is.na(rf_rast)] <- rf_prediction
plot(rf_rast)
writeRaster(rf_rast, "rf_rast.tif")

#  xg_boost
xgb_prediction = predict(xg_model,datastackrzdf)
xgb_rast=refrast
xgb_rast[!is.na(xgb_rast)] <- xgb_prediction
plot(xgb_rast)
writeRaster(xgb_rast, "xgb_rast.tif")

#  neunet
# Standardize data first
datastackrzdfx=data.frame(apply(datastackrzdf, 2, FUN=zscore))
neunet_prediction = predict(neunet_model,datastackrzdfx)
# Values need to be rescaled again
neunet_prediction =neunet_prediction *sd(train$agb)+mean(train$agb)
neunet_rast=refrast
neunet_rast[!is.na(neunet_rast)] <- neunet_prediction
plot(neunet_rast)
writeRaster(neunet_rast, "neunet_rast.tif")

#  nnet
nnet_prediction = predict(nnet_model,datastackrzdfx)
# Values need to be rescaled again
nnet_prediction =nnet_prediction *sd(train$agb)+mean(train$agb)
nnet_rast=refrast
nnet_rast[!is.na(nnet_rast)] <- nnet_prediction
plot(nnet_rast)
writeRaster(nnet_rast, "nnet_rast.tif")
```
