...
wd=("/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/MALESA/2022Fall/Week11_Tunning")
setwd(wd)
#library(terra)
#library(sf)
#library(ggplot2)
...
tempdir="/Users/tug61163/Downloads/tempdir"
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
tunegrid <- expand.grid(.mtry = c(sqrt(ncol(refdata))))
modellist <- list()
for (ntree in c(10,500, 1000)){
  set.seed(123)
  fit <- train(veg~.,
               data = train,
               method = 'rf',
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
metric="Accuracy" ### Accuracy for supervised classification
...
test_y = test[, 17]
confusionMatrix(pred_y, test$veg)
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
#use model to make predictions on test data
pred_y1 = predict(model, test)
# Produce performance metrics
test_y = test[, 17]
confusionMatrix(pred_y1, test$veg)
```
...
Perform an accuracy assessment for both algorithms using the test set and report your accuracy results. Based on your results, explain what algorithm performs  best.