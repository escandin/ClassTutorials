...
summary(refdata)
refdata=refdata[, -2] # remove the line that corresponds to ID
```

### 2.  Split for Training, Validation, and Testing
```{r include = TRUE, message=F, warning=F, eval=FALSE}
set.seed(123)
data_split <- initial_split(refdata, prop = 0.8)
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
recipe_agb <- recipe(agb ~ ., data = train_data_final) %>%
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
show_best(rf_tuned, metric="rmse") # this shows the top 5 models
rf_best <- select_best(rf_tuned, metric="rmse") # this selects the very best model
rf_best_simp <- select_by_one_std_err(rf_tuned, metric="rmse", min_n)

show_best(xgb_tuned, metric="rmse") 
xgb_best <- select_best(xgb_tuned, metric="rmse")

show_best(nn_tuned, metric="rmse")
nn_best <- select_best(nn_tuned, metric="rmse")
```
