...
```{r include = FALSE}
# Load screenshots
wd="/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/IntroRemoteSensing/2023Fall/Class2/LabMaterials"
...
5  Click on the results tab and locate the Landsat image specified below. As you can see in the grayed image below, it corresponds to a scene acquired on April 10 2023.
...
Click on the "Add All to Bulk" button and then close. 
```{r include = TRUE, echo=FALSE}
...
Check the settings of your browser to make sure it does not block pop-up windows by entering this address to our Chrome browser: chrome://settings/content/popups . Make sure that https://earthexplorer.usgs.gov/ is added under "Allow.ed to send pop-ups and use redirects"
...
e=draw()
L8rsz=crop(L8,e)
plotRGB(L8rsz, r=3, g=2, b=1, stretch="lin") # true color composite
```

6.
Let's visualize band composites with other band combinations 
```{r include = TRUE, message=F, warning=F, eval=FALSE}
plotRGB(L8rsz, r=4, g=3, b=2, stretch="lin") # Infrared composite
plotRGB(L8rsz, r=5, g=4, b=3, stretch="lin") # SWIR composite
```
...
Let's visualize band composites with other band combinations 
```{r include = TRUE, message=F, warning=F, eval=FALSE}
plotRGB(L8rsz, r=4, g=3, b=2, stretch="hist") # Infrared composite
plotRGB(L8rsz, r=5, g=4, b=3, stretch="hist") # SWIR composite
```
...
pdf("VGutierrez_Lab2.pdf")
  plotRGB(L8rsz, r=3, g=2, b=1, stretch="lin") 
  plotRGB(L8rsz, r=4, g=3, b=2, stretch="lin") # Infrared composite
  plotRGB(L8rsz, r=5, g=4, b=3, stretch="lin") # SWIR composite
dev.off()
```
...
1.Go to Earth Explorer and select a new location: Type the name of your home town, use the same date range as before (06/01/20221 and 09/01/2023) and cloud cover less than 10%. Select the image that looks best to you (that means not too many clouds, clear view) (0.5pts)
...
4. Adapt the code written in step B.5. to crop the image to the extent of the biggest urban area that you can find in the image  

5. Use the plotRGB() function in step B.6 to produce a plot with bands 6, 5, and 4 and the argument stretch="lin"  Make sure that your polygon does not include any black background pixels in the edges of the image. Do not modify anything else including parenthesis or quotation marks, otherwise the code will not run. 
...
5. Produce a single pdf file with two plotRGB maps representing a false composite with bands 5,4,3 using "lin" and "hist"stretching  and upload the pdf to canvas with your first initial, last name and lab number (see notation in step B.8.). In order to plot the three images in the same pdf file, write the two plotRGB functions together between the pdf() funtion and the dev.off() function (2 pts).

Discuss briefly the differences in the visualization properties between the two images. Which option enables a better visualization of the different features in the image (e.g. urban areas, rivers, forests, grasslands)?
