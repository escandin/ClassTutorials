...
```{r include = FALSE}
# Load screenshots
wd="/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/IntroRemoteSensing/2023Fall/Class1/RS_Lab1_2023"
...
If you receive an error such as "Error in library(library name) : there is no package called (library name)", please install the library, such as (make sure you write the library name with quotation marks:
```{r include = TRUE, message=F, warning=F, eval=FALSE}
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
setwd("/Users/tug61163/Library/CloudStorage/OneDrive-TempleUniversity/Courses/IntroRemoteSensing/2023Fall/Class1/RS_Lab1_2023")
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
persp(DEM)
persp(DEM, theta=90, phi=0) 
persp(DEM, theta=60, phi=0) 
```

Step 4: Produce a level plot to observe the image and the profiles simultaneously. 
```{r include = TRUE, message=F, warning=F, eval=FALSE}
levelplot(DEM, col.regions=terrain.colors)
```
...
Landsat=rast("kauai_niihau_lehua2.tif")
```
...
plotRGB(Landsat, r=1, g=2, b=3, stretch="lin")
```
...
# notice that the dimensions of the original file and the resized one are different
Landsat
LandsatRsz

# Let's plot the resized object
...
## Instructions
Edit the code in steps 1 to 5 to download, unzip, import and plot a map with the DEM and another with the Landsat RGB image produced for the island of Oahu instead of Kauai.

Remember to execute "all" the lines in order, making sure to make the edits specified below before the execution. 

System setup: replace the route for the working directory that appears in the function setwd() for a route to a working file that you selected (e.g. "/Users/tue27721/Documents/Courses/IntroRemoteSensing/2023Fall/Class1/RS_Lab1_2023" in mac or

"c:/Users/Documents/Courses/IntroRemoteSensing/2023Fall/Class1/RS_Lab1_2023" 

Alternatively you can go to the RStudio menu and select Session > Set Working Directory > Choose Directory

Step 1: Change the URL in the downloadURL function for this one "ftp://soest.hawaii.edu/coastal/webftp/Oahu/dem/Oahu_DEM.zip" , to download data from the island of Oahu instead of Kauai.

Change the string (text) stored in the object named DEMfilename to "Oahu_DEM.zip" 
...
Steps 3 and 4: Run the code to produce perspective plots for Oahu.

Step 5: Similarly to step 1, change the URL in the downloadURL function for this one "ftp://soest.hawaii.edu/coastal/webftp/Oahu/lansat/geotiff.zip"

Step 6: run as is.

Step 7: This step is not necessary because the image already comes cropped to the size of the island.

Step 8: Change the name of the pdf file for your Last_nameFirst_initial and the name of the assignment (e.g. "GutierrezV_Lab1.pdf"").
