...
### SETTING UP THE ENVIRONMENT AND UPLOADING DATA
...
5. Interpret the results and draw conclusions
The p-level is very low (and therefore the confidence intervals at a very high level do not include 50). Therefore results support the alternative hypothesis that the favorabilty for Trump among US voters is different to 50. Results also suggest that favorability towards Trump is lower than 50%.

#### Excercise 2: one tailed test
Perform the same exercise than above but this time test whether TRUMP support is lower than 50%.
...
The standard deviations are very similar so they can be assumed as the same. A more rigurous way to assess equally of variance is through the application of an F-test. We will cover the F-test later. For now, let's say that the F-test assesses the probability of two variances being different.
```{r include = TRUE, message=F, warning=F, eval=FALSE}
...
5. Interpret the results and draw conclusions:
The p-level is very low (and therefore the confidence intervals at a very high level of confidence do not include 0). Therefore results support the alternative hypothesis that the favorability for Trump among US male and female voters is different. Results also suggest that female see Trump less favorable than male.
...
Upload your answers as a word document in module 7 in canvas.
