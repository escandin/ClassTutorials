...
## Goal
At the end of this lab, students will know how to evaluate modeling assumptions in linear regression analysis through visual interpretation.
...
### Let's produce a visualization with synthetic data
...
### Diagnose linear regression with data from UN-IHD
...
#### Exercise 2. extreme observations, heteroskedasticity
```{r include = TRUE, message=F, warning=F, eval=FALSE}
...
### Lab report
1. Expore the names of variables available at the UN-HDI dataset. Then select a variable that you want to predict. Then select three potential variables that you think might be good predictors in a linear regression model. Respond to the questions below:

    1a. What is the selected to be predicted?
    
    1b. What are the three selected potential predictors

    1c. What is the expected relationship between the response and each one of these variables?  Do you expect the relationship to be positive or negative and why? 
...
2. Produce a scatterplot for the dependent variable and each one of the three pre-selected predictors. Include the three scatterplots in your report.

3. Select the variable that you think might best predict the response and use it to produce a linear regression. Provide the following information in your report:
...
3. Produce a series of diagnostics graphs for the model (>plot(mod)). Interpret each graph to  diagnose:
    ...
    3a. linearity of relationship
    
    3b. Normality and independence of errors

    3c. homo(hetero)sedasticity

    3d. Extreme observations
