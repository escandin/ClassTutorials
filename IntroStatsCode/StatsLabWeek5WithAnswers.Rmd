...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
n=40
CIlow=meanHHinc-qnorm(0.99)*sdHHinc/sqrt(n)
CIhi=meanHHinc+qnorm(0.99)*sdHHinc/sqrt(n)
...
CIlow=p-pnorm(0.975)*sqrt(p*(1-p)/n)
CIhi= p+pnorm(0.975)*sqrt(p*(1-p)/n)
```
...
p=pnorm(q, meanHomic, sdHomic, lower.tail=FALSE)

# Graph
y=seq(meanHomic-(3*sdHomic), meanHomic+(3*sdHomic), by =1)
probval=dnorm(y, meanHomic, sdHomic)
plot(y, probval, type="l")
abline(v=q, col="red")
abline(h=dnorm(q, meanHomic, sdHomic), col="green")
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
pnorm(45, meanHomic, sdHomic/sqrt(20), lower.tail=FALSE)

qmean=45
# Graph
y=seq(meanHomic-(5*sdHomic/sqrt(20)), meanHomic+(5*sdHomic/sqrt(20)), by =0.1)
probval=dnorm(y, meanHomic, sdHomic/sqrt(20))
plot(y, probval, type="l")
abline(v=qmean, col="red")
```
...
```{r include = TRUE, message=F, warning=F, eval=FALSE}
pnorm(45, meanHomic, sdHomic/sqrt(20))-pnorm(35, meanHomic, sdHomic/sqrt(20))
...
1.	To estimate the average age of NFL players, a research group randomly surveyed 200 NFL players from different teams in 2020. The below grouped frequency table shows their survey result. 
Age	Counts
15-20	35
20-25	38
25-30	48
...
-First, calculate the mean and SD from the sample (above table). You can check how to do that here: From here: https://www.statology.org/histogram-standard-deviation/   

-Determine whether you should use z-distribution or t-distribution. 

-Next, find the appropriate formula. Find the correct t-value (or z-value) for your significance level of 98% (two-tailed). Calculate the Standard Error, and the EBM. 
...
-Write your interpretation stating what your CI mean.

```{r include = TRUE, message=F, warning=F, eval=FALSE}
AgeBin=c(seq(17.5, 37.5, by=5))
counts=c(35, 38, 48, 45, 34)
meanVal=sum(AgeBin*counts)/sum(counts)
# or
meanVal=sum(AgeBin*counts/sum(counts))

AgeDev=AgeBin-meanVal # Deviation from each bin to the mean
sdVal=sqrt(sum(counts*AgeDev^2)/(sum(counts)-1)) 

LowCI=meanVal-qnorm(0.99)*sdVal/sqrt(sum(counts))
HiCI=meanVal+qnorm(0.99)*sdVal/sqrt(sum(counts))

# Graph
y=seq(meanVal-(3*sdVal), meanVal+(3*sdVal), by =1)
probval=dnorm(y, meanVal, sdVal)
plot(y, probval, type="l")
abline(v=LowCI, col="red")
abline(v=HiCI, col="green")
```

How does your result compare when you consider a 90% confidence interval? How do you interpret the differences?
```{r include = TRUE, message=F, warning=F, eval=FALSE}
LowCI=meanVal-qnorm(0.95)*sdVal/sqrt(sum(counts))
HiCI=meanVal+qnorm(0.95)*sdVal/sqrt(sum(counts))
abline(v=LowCI, col="blue")
abline(v=HiCI, col="yellow")
```

Can you use 100% confidence interval? Try inputting 1 or 0 as the CI. What value do you get in R and why?
```{r include = TRUE, message=F, warning=F, eval=FALSE}
meanVal+qnorm(1)*sdVal/sqrt(sum(counts))
meanVal+qnorm(0)*sdVal/sqrt(sum(counts))
```

#### Part 3: CI of Proportion
Suppose, 54% of 500 randomly surveyed respondents said that they voted Biden in the 2020 election. Calculate the 97% confidence interval of Americans who voted for Biden.
...
CIlow=p-qnorm(0.985)*sqrt(p*(1-p)/n)
CIhi=p+qnorm(0.985)*sqrt(p*(1-p)/n)
```
